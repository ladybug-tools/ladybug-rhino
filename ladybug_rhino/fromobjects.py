...
from .text import grasshopper_text


def grasshopper_legend(legend):
    ...
    Returns:
        A list of Rhino geomtries in the following order:
    ...
    legend_title = grasshopper_text(legend.title, legend.title_location, _height, _font)
    if legend.legend_parameters.continuous_legend is False:
        legend_text = [grasshopper_text(txt, loc, _height, _font) for txt, loc in
                       zip(legend.segment_text, legend.segment_text_location)]
    elif legend.legend_parameters.vertical_or_horizontal is True:
        legend_text = [grasshopper_text(txt, loc, _height, _font, 0, 3) for txt, loc in
                       zip(legend.segment_text, legend.segment_text_location)]
    else:
        legend_text = [grasshopper_text(txt, loc, _height, _font, 1, 5) for txt, loc in
                       zip(legend.segment_text, legend.segment_text_location)]
